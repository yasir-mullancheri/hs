...
// ===== PRICING CONFIG =====
// Base plan prices per device
const PLAN_PRICES = {
    yearly: 1000,
    monthly: 100
};

// Addon prices per device per plan
// Yearly: all addons are $200 each
// Monthly: Automation+ = $25, Experience+ = $75, Performance+ = $75
const ADDON_PRICES = {
    yearly: {
        automation: 200,
        experience: 200,
...
// Bundle discount: $50 off per device ONLY when all 3 addons selected
const BUNDLE_DISCOUNT = 50; // applied per device when all 3 addons selected
...
/**
 * Update the displayed price on each addon card based on selected plan.
 * Yearly: all $200/device/year
 * Monthly: Automation+ = $25, Experience+ = $75, Performance+ = $75
 */
...
/**
 * Calculate total addon cost for the given device count.
 * - Prices are per-addon (not flat rate for monthly).
 * - $50/device discount only when ALL 3 addons selected.
 * - No discount for 1 or 2 addons.
 */
function calculateAddonTotal(deviceCount) {
    ...
    // Sum up the individual prices for selected addons
    let sumPerDevice = selectedAddons.reduce((sum, addon) => {
        return sum + (prices[addon] || 0);
    }, 0);
    
    // Apply $50/device discount only when all 3 addons are selected
    if (selectedAddons.length === 3) {
        sumPerDevice -= BUNDLE_DISCOUNT;
    }
...
/**
 * Calculate the per-device addon price (used for FoxyCart field and summary display).
 */
...
function updatePricingSummary() {
    ...
    // Build addon description for summary
    ...
    if (selectedAddons.length > 0) {
        ...
        const selectedAddonNames = selectedAddons.map(addon => addonLabels[addon]);
        const addonPricePerDevice = getAddonPricePerDevice();
        
        let discountNote = '';
        if (selectedAddons.length === 3) {
            discountNote = ' <em>($50/device bundle discount applied)</em>';
        }
...
function populateFoxyCartFields() {
    ...
    document.getElementById('fc-sub-frequency').value = selectedPlan === 'monthly' ? '1m' : '1y';
    ...
    if (selectedAddons.length > 0) {
        const addonProductName = `Add-ons: ${selectedAddonNames.join(', ')} - ${billingCycle.charAt(0).toUpperCase() + billingCycle.slice(1)} (per device)`;
...
function loadFinalSummary() {
    ...
    if (orderItems) {
        ...
        if (selectedAddons.length > 0) {
            ...
            let discountNote = '';
            if (selectedAddons.length === 3) {
                discountNote = `<div class="item-detail" style="color: var(--success-color, #22c55e);">$50/device bundle discount applied</div>`;
            }
            ...
            itemsHTML += `
                <div class="item-row">
                    <div class="item-info">
                        ...
                        <div class="item-description">${deviceCount} ${deviceCount === 1 ? 'device' : 'devices'} × $${addonPricePerDevice}/device – ${selectedPlan === 'yearly' ? 'Yearly' : 'Monthly'}</div>
                        <div class="item-details">
